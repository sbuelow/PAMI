...
* Full name: `\PAMI\Message\Action\DongleRestartAction`
...
```php
public __construct(string $when, string $device): void
```
...
|-----------|------------|--------------------------------------|
| `$when`   | **string** | now \| gracefully \| when convenient |
| `$device` | **string** | Dongle device name.                  |
